...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['signup'])) {
    ...
    if (isset($_POST['email']) && isset($_POST['psw']) && isset($_POST['rpsw'])) {
        ...
        if (preg_match('/^[a-zA-Z0-9._%+-]+@gmail\.com$/', $email)) {
            ...
            if ($password == $rpassword) {
                ...
                if ($result->num_rows > 0) {
                    ...
                    if ($stmt->execute()) {
                        ...
                        echo '<script>alert("Signup failed"); window.location.href = "admin.php";</script>';
                        exit();
                    }
                }
            } else {
            ...
            echo '<script>alert("Invalid email format. Please enter a Gmail address"); window.location.href = "admin.php";</script>';
            exit();
        }
    } else {
        // All signup fields not provided, display a message and redirect to signup page
        echo '<script>alert("All signup fields are required"); window.location.href = "admin.php";</script>';
        exit();
    }
}
